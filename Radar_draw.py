...
class DB:
    ...
    def check(p_id, mode):      # check database is non have this tuple
        ...
        df_sql = pd.read_sql('SELECT * FROM player_radar', con=connection)      # get 'badminton.player_radar' table
        p_id = int(p_id)
        mode = str(mode)
    ...
    def insertBLOB(p_id, mode, photo):    # image to DB
        if DB.check(p_id, mode) : return     # Determine if it already exists
...
class Game:
    def __init__(self):     # constructor
        ...
        Game.radar('Player0',  df_sql, '0')  # draw radar from dataframe (condition)
        Game.radar('Player11', df_sql, '11')
        Game.radar('Player16', df_sql, '16')
        Game.radar('Player17', df_sql, '17')
        Game.radar('Player20', df_sql, '20')
        Game.radar('Player25', df_sql, '25')
        Game.radar('Player29', df_sql, '29')
        #db.close()
