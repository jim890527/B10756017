...
import real_time_detection as yolo
...
class DB:
    ...
    def drop_player_radar():
        ...
        if messagebox.askyesno(title='Drop', message='Drop player radar Table ?'):
            name = 'player_radar'
            DB.drop_table(name)
        else:
            return
...
class Game:
    ...
    def add_radar(target, color, ax, angles, df_r, normalize):
        condition = df_r['target'] == target
        df_ar = df_r.loc[condition]      #number = numberr and target = input
        #print(df_ar)
        df_ar = df_ar.groupby(['target']).mean()
        #print(df_ar)
        df_ar['start_to_light'] = df_ar['start_to_light'] / normalize
        df_ar['light_to_start'] = df_ar['light_to_start'] / normalize
        #print(df_ar)
    ...
    def radar(table, df_r, numberr):   # table = title name
        ...
        if count[0] != 6:
            ...
            Game.add_radar('1', '#FF2D2D', ax, angles, df_r, 4)
            Game.add_radar('2', '#FF8040', ax, angles, df_r, 4)
            Game.add_radar('3', '#FFD306', ax, angles, df_r, 4)
            Game.add_radar('4', '#00DB00', ax, angles, df_r, 4)
            Game.add_radar('5', '#66B3FF', ax, angles, df_r, 2.6)
            Game.add_radar('6', '#B15BFF', ax, angles, df_r, 2.6)
...
class video:
    def __init__(self, name):     # constructor
        if name != '': yolo.main(name)     # video identify 
        df = pd.read_excel("timer.xlsx")
        #df.dropna(axis=1, inplace=True) 
        conn = DB.creat(df,'video')   # create table
        print('\r\n')
        df_sql = pd.read_sql('SELECT * FROM video', con=conn)   # select table to pandas
        df_sql = df_sql[['name', '2-5', '3-4', '3-6', '4-5', '5-4', '8-7']]
        #Game.to_eng(df_sql)  # dataframe translate to english
        n = df_sql['name'].unique()   # get number
        for x in n:
            video.radar('Player_' + str(x), df_sql, str(x))    # draw radar from dataframe (condition)
        #db.close()
    
    def add_radar(name, color, ax, angles, df_r):
        values = df_r.loc[name].values.tolist()
        #print(values[1])
        values += values[:1]
        #print(values)
        # Draw the outline of our data.
    ...
    def radar(table, df_r, name):   # table = title name
        condition = df_r['name'] == name   # number = numberr
        df_r = df_r.loc[condition]
        df_gr = df_r.groupby(['name']).mean() 
        print(df_gr)
        count = df_gr.groupby(['name']).size()
        #print(count[0])
        if count[0] == 0:
            print('ERROR!')
            print('\r\n')
            ...
            print('\r\n')
            label = ['2-5','3-4','3-6','4-5','5-4','8-7']  #draw data col
            num = len(label)
            angles = np.linspace(0, 2 * np.pi, num, endpoint=False).tolist()
            angles += angles[:1]
            label += label[:1]
            fig, ax = plt.subplots(figsize=(6, 6), subplot_kw=dict(polar=True))
            video.add_radar(name, '#66B3FF', ax, angles, df_gr)
            ax.set_theta_offset(np.pi / 2)
            ax.set_theta_direction(-1)
            #print(np.degrees(angles))
            ax.set_thetagrids(np.degrees(angles), label)
            ...
            ax.set_title(str(table),size=15)
            #ax.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1))
            plt.savefig(fname=r'video/'+str(table)+'.png', bbox_inches='tight', dpi=150)
            DB.insertBLOB(name, "video", r'video/'+str(table)+'.png')  # img to database
...
class UI:
    ...
    def tools():
        ...
        root.iconbitmap("ball.ico")   # ico
        root.title('戴姿穎我愛妳')
        root.geometry('300x578')    # x,y
        tk.Label(root,text="操作介面",bg='#1ABC9C',font=('Arial',20) ,width=300,height=3,fg='#FFFFFF').pack(side="top")
        en1 = tk.Entry(root,width=10)
        en1.place(x=113,y=80)
        tk.Button(root, text='影像辨識',command=lambda:video(str(en1.get())),bg='#1ABC9C',width=40,height=3,font=('Arial',12),fg='#FFFFFF').pack(side="top",fill='x')
        tk.Button(root, text='上傳比賽模式雷達圖' ,command=lambda:Game(),bg='#1ABC9C',width=40,height=3,font=('Arial',12),fg='#FFFFFF').pack(side="top",fill='x')
        tk.Button(root, text='上傳T字型模式雷達圖', command=lambda:T(),bg='#1ABC9C',width=40,height=3,font=('Arial',12),fg='#FFFFFF').pack(side="top",fill='x')
        tk.Button(root, text='上傳米字型模式雷達圖',command=lambda:Mi(),bg='#1ABC9C',width=40,height=3,font=('Arial',12),fg='#FFFFFF').pack(side="top",fill='x')
        tk.Button(root, text='清除所有雷達圖資料',command=lambda:DB.drop_player_radar(),bg='#1ABC9C',width=40,height=3,font=('Arial',12),fg='#FFFFFF').pack(side="top",fill='x')
        tk.Button(root, text='連結網頁', command=lambda:DB.open_url(),bg='#1ABC9C',width=40,height=3,font=('Arial',12),fg='#FFFFFF').pack(side="top",fill='x')
        tk.Button(root, text='Close', command=lambda:UI.quit(root),bg='#1ABC9C',width=40,height=3,font=('Arial',12),fg='#FFFFFF').pack(side="top",fill='x')
        root.mainloop()
    
    def login(user, password, inter):
        if DB.check(user, password, 'user'):
            print('login')
            inter.destroy()
            UI.tools()
        else:
            messagebox.showerror("Error", "Failed to login")  #標題，訊息
            print('failed to login')
    
    def login_window():
        tkWindow = tk.Tk()
        tkWindow.resizable(False,0)
        tkWindow.iconbitmap("ball.ico")   # ico
        tkWindow.geometry('400x300')
        tkWindow.title('戴資穎我愛妳')
        img = Image.open('badminton.png')
        img = img.resize((img.width//20,img.height//20))
        imgtk = ImageTk.PhotoImage(img)
        tk.Label(tkWindow,image=imgtk,bg='#1ABC9C').pack(side="left",fill='y')     # logo
        tk.Label(tkWindow, text = 'Account',font=('Arial',15)).pack(pady=10)
        ent1 = tk.Entry(tkWindow,width=20)
        ent1.pack(padx=20,pady=6)
        tk.Label(tkWindow, text = 'Password',font=('Arial',15)).pack(pady=10)
        ent2 = tk.Entry(tkWindow,width=20)
        ent2.pack(padx=20,pady=6)
        tk.Button(tkWindow, text="Login", command = lambda:[UI.login(ent1.get(),ent2.get(),tkWindow)],width=20,height=1,font=('Arial',10)).pack(pady=15)
        tk.Button(tkWindow,text="Close", command = lambda:[UI.quit(tkWindow)],width=20,height=1,font=('Arial',10)).pack(pady=10)
        tkWindow.mainloop()
        
def main():
    UI()
    #video(str(input('please input palyer name: ')))
