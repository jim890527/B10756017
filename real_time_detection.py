...
def video_init(is_2_write=True, save_path= r'test.mp4'):
    writer = None
    cap = cv2.VideoCapture('badminton_rotated.mp4')
    ...
    if is_2_write is True:
        ...
        if save_path is None:
            save_path = r'test.mp4'
        writer = cv2.VideoWriter(save_path, fourcc, 60, (int(width), int(height)))
...
class Yolo_v4():
    def __init__(self, model_path, GPU_ratio=0.6):
        #----var
        class_num = 80
        height = 608 #416, 608
        width = 608 #416, 608
    ...
    def detection(self, img_bgr):
        ...
        img_bgr, self.c , self.time_start = tools.draw_img(img_bgr, boxes, score, label, self.label_dict, self.color_table, self.c, self.time_start)
        
        return img_bgr

def real_time_obj_detection(name ,model_path, GPU_ratio=0.4):
    #----var
    frame_count = 0
    FPS = "60"
    d_t = 0
    ...
    while (cap.isOpened()):
        ...
        if ret is True:
            ...
            #check(img.shape[1], img.shape[0], cx, cy)
            ...
            #----image display
            cv2.imshow("YOLO v4 by JunYi", img)
    ...
    tools.to_excel(name,0,0)   # number
    #----release
    yolo_v4.sess.close()
    cap.release()
    if writer is not None:
...
def main(name):
    ...
    #save_path =  r"test.mp4"
    GPU_ratio = 0.6
    real_time_obj_detection(name, model_path, GPU_ratio=GPU_ratio)
